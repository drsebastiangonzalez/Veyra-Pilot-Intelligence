...
(() => {
  ...
  A.validateRecord=(r,g)=>{
    ...
    for(const c of selected){if(!A.names[c.code])throw Error('Competencia no válida.');if(!c.evidence?.trim()||c.evidence.trim().length<8)throw Error('Añade la observación que respalda '+c.code+'.');}
