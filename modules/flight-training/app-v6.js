globalThis.VeyraFlightTrainingStart=()=>{
  ...
  const state={evaluation:'eval1',scenario:'s1',person:'CM1',index:0,screen:'event',drafts:{},records:[],positions:{},counter:0,deleted:null,guideOpen:false,setupOpen:false};
  ...
  function layout(){L.apply(document.body);window.dispatchEvent(new Event('veyra:layout'));}
  function capture(){
  ...
  function reference(group){
    ...
    return '<section class="phase-reference"><div class="compact-phase"><h2>'+esc(group.title)+'</h2><span class="phase-counter">'+(state.index+1)+' / '+sequence().length+'</span></div><details id="sessionGuide"'+(state.guideOpen?' open':'')+'><summary>Session Development / Instructor Focus</summary><div class="guide-columns"><section class="development-column"><h3>Session Development</h3>'+column('development')+'</section><section class="focus-column"><h3>Instructor Focus</h3>'+column('focus')+'</section></div></details><details id="sessionSetup"'+(state.setupOpen?' open':'')+'><summary>Configuración de la sesión</summary><div class="reference">'+(page?table(page.setup)+table(page.comms)+list(page.cautions):'')+'</div></details></section>';
  }
  function eventView(){
    ...
    $('analyzeButton').onclick=()=>{capture();if(!d.text.trim()){toast('Escribe una observación del piloto.');return;}d.original=d.text;d.analysis=A.analyze(d.text,G,current());d.revision=d.analysis.corrected;renderRevision();renderCompetencies();layout();};
  ...
  function renderCompetencies(){
    ...
    $('competencyPanel').innerHTML='<div class="field-head"><h3>Competencias del piloto</h3><span class="selection-count">'+d.competencies.length+' / 9</span></div><p class="competency-legend"><span class="focus-dot"></span><span>Foco del instructor</span><span class="suggested-dot"></span><span>Propuesta</span></p><div class="competency-list">'+Object.entries(A.names).map(([code,name])=>{
      ...
      return '<div class="competency-item '+(focus.has(code)?'in-focus ':'')+(selected?'is-selected ':'')+(suggested.has(code)?'is-suggested':'')+'"><div class="competency-row"><button type="button" data-competency="'+code+'" aria-pressed="'+selected+'" class="competency-toggle"><span class="choice-mark" aria-hidden="true">'+(selected?'✓':'+')+'</span><strong>'+code+'</strong><span>'+esc(name)+'</span></button><button type="button" data-toggle-ob="'+code+'" aria-expanded="'+open+'" aria-controls="obs-'+code+'" class="ob-toggle">OB'+(selectedCount?' '+selectedCount:'')+' '+(open?'▴':'▾')+'</button></div><div class="ob-drawer" id="obs-'+code+'"'+(!open?' hidden':'')+'>'+obs.map(o=>{
  ...
  function exportJSON(){download(JSON.stringify({version:'0.6',language:L.language,target:'pilot',guideVersion:G.version,records:state.records,drafts:state.drafts,exportedAt:new Date().toISOString()},null,2),'Veyra_Flight_Training_Evaluacion.json','application/json');}
  function exportText(){
  ...
  window.addEventListener('beforeunload',e=>{if(state.records.length||Object.values(state.drafts).some(d=>d.text.trim())){e.preventDefault();e.returnValue='';}});
  render();
};
